...
const RegisterForm = () => {
  ...
  const { email, password, name, date, loading, error } = useSelector(
  ...
  const handleRegister = async (e: { preventDefault: () => void }) => {
    ...
    dispatch(loginStart())
    try {
      const response = await fetch('http://localhost:9002/api/user/register', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(customer.message || 'Register failed')
      }
    ...
    <>
      ...
      <div className='ml-[35%] mt-20'>
        <div className='flex flex-col justify-center items-center bg-white shadow-2xl w-[600px] p-10'>
          <div className='text-3xl font-bold leading-[35px]'>Sign up</div>
